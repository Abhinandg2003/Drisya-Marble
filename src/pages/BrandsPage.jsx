...
function FilterCarousel({ label, items, activeKey, onSelect, getKey, getLabel }) {
  ...
  return (
    <div className="row items-center gap-2 mb-12 pb-6 border-b border-[#D4B896]/30">
      ...
      <div className='flex'>
        <button onClick={() => scroll(-1)} className="flex-shrink-0 w-9 h-9 m-2 flex items-center justify-center border border-[#D4B896]/60 text-[#5A4E44] hover:border-[#782423] rounded-md hover:text-[#782423] transition-colors">‹</button>
        <div ref={scrollRef} className="flex items-center gap-2 overflow-x-auto flex-1" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          <button onClick={() => onSelect('all')} className={`flex-shrink-0 px-5 py-2 text-[12px] rounded-md tracking-wide transition-colors border ${activeKey === 'all' ? 'border rounded-md bg-gradient-to-r from-[#711a19] via-[#7e2e1e] to-[#711a19] text-[#fff6e2] transition-all duration-200' : 'border-[#D4B896]/60 text-[#5A4E44] hover:border-[#711a19] rounded-md hover:text-[#782423]'}`}>All</button>
          {items.map(item => (
            <button key={getKey(item)} onClick={() => onSelect(getKey(item))} className={`flex-shrink-0 px-5 py-2 text-[12px] rounded-md tracking-wide transition-colors border ${activeKey === getKey(item) ? 'border rounded-md bg-gradient-to-r from-[#711a19] via-[#7e2e1e] to-[#711a19] text-[#fff6e2] transition-all duration-200' : 'border-[#D4B896]/60 text-[#5A4E44] rounded-md hover:border-[#711a19] hover:text-[#782423]'}`}>{getLabel(item)}</button>
          ))}
        </div>
        <button onClick={() => scroll(1)} className="flex-shrink-0 w-9 m-2 h-9 flex items-center rounded-md justify-center border border-[#D4B896]/60 text-[#5A4E44] hover:border-[#782423] hover:text-[#782423] transition-colors">›</button>
      </div>
    </div>
  )
}
...
export default function BrandsPage() {
  ...
  const setBrand = (name) => {
    ...
    else setSearchParams({ name })
  }

  const filtered = activeBrand === 'all'
    ? productItems
    : productItems.filter(item => item.brand === activeBrand)

  return (
    <main className="pt-20 bg-[#fdfaef]">
      <div className="relative h-[30vh] md:h-[50vh] overflow-hidden">
        <picture>
          <source srcSet="/images/brands2.avif" type="image/avif" />
          <img src="/images/brands2.jpg" alt="Brands" className="absolute inset-0 w-full h-full object-cover" />
        </picture>
        <div className="relative z-10 h-full flex flex-col justify-center px-8 md:px-16">
          <h1 className="font-display text-[35px] md:text-[70px] text-white">Browse by Brands</h1>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filtered.map((item, idx) => (
            <Link key={item.slug ? `${item.slug}-${idx}` : idx} to={item.slug ? `/products/${item.slug}` : '#'} className="group">
              <div className="aspect-square overflow-hidden rounded-sm mb-4 bg-[#E8DDD4] relative">
      ...
      <div className='mb-10'>
        <h3 className='flex text-3xl font-medium font-display justify-center mx-10 text-center '>
          Can't Find What You are Looking for?
        </h3>
        <div className='justify-center flex'>
          <Link to="/products">
            <button className='w-auto bg-gradient-to-r rounded-md from-[#711a19] via-[#7e2e1e] to-[#711a19] m-5 text-white text-[14px] tracking-wide py-3 px-5  flex items-center justify-center gap-3 '> Explore Collections</button>
          </Link>
        </div>
      </div>
    </main>
